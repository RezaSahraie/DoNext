    ...
    <section
        ...
        <div class="px-6 pb-6">
            ...
            <div class="-mt-14 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
                ...
                {{-- Avatar --}}
                <div
                    class="grid h-28 w-28 place-items-center rounded-3xl border-8 border-white bg-indigo-100 text-4xl font-black text-indigo-700 shadow-xl dark:border-slate-900 dark:bg-indigo-500/20 dark:text-indigo-300">
                ...
                @if ($editMode)
                    <button type="button" wire:click="cancelEdit"
                        class="rounded-xl border border-slate-200 px-4 py-2.5 text-xs font-bold transition hover:bg-slate-50 dark:border-slate-700 dark:hover:bg-slate-800">
                        انصراف
                    </button>
                @else
            ...
            <p class="mt-1 text-sm text-slate-400">
                {{ $user->email }}
                ·
                عضو از {{ $user->created_at->format('Y') }}
            </p>
    ...
    <div class="grid gap-6 lg:grid-cols-3">
        ...
        <section
            ...
            <div class="flex items-center justify-between">
                ...
                @if ($editMode)
                    <span
                        class="rounded-full bg-indigo-50 px-3 py-1 text-[10px] font-bold text-indigo-600 dark:bg-indigo-500/10 dark:text-indigo-400">
                        در حال ویرایش
                    </span>
                @endif
            </div>
                ...
                <form wire:submit="updateProfile" class="mt-6 space-y-5">
                    ...
                    <div class="grid gap-4 sm:grid-cols-2">
                        ...
                        <div>
                            ...
                            <input id="name" type="text" wire:model="name"
                                class="mt-2 h-11 w-full rounded-xl border border-slate-200 bg-white px-4 text-sm outline-none transition focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 dark:border-slate-700 dark:bg-slate-800 dark:text-white"
                                placeholder="نام شما">
                    ...
                    <div class="flex gap-3 pt-2">
                        ...
                        <button type="submit" wire:loading.attr="disabled" wire:target="updateProfile"
                            class="rounded-xl bg-indigo-600 px-5 py-2.5 text-sm font-bold text-white transition hover:bg-indigo-700 disabled:cursor-not-allowed disabled:opacity-60">
                        ...
                        <button type="button" wire:click="cancelEdit"
                            class="rounded-xl border border-slate-200 px-5 py-2.5 text-sm font-bold transition hover:bg-slate-50 dark:border-slate-700 dark:hover:bg-slate-800">
                            انصراف
                        </button>
