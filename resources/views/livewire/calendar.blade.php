    ...
    <section class="flex flex-col gap-5 md:flex-row md:items-end md:justify-between">
        <div>
            <p class="mb-2 text-sm font-semibold text-indigo-600 dark:text-indigo-400">
                <span x-show="lang === 'fa'">برنامه‌ریزی روزها</span>
                <span x-show="lang === 'en'">Plan your days</span>
            </p>
            <h2 class="text-3xl font-black tracking-tight sm:text-4xl">
                <span x-show="lang === 'fa'">تقویم</span>
                <span x-show="lang === 'en'">Calendar</span>
            </h2>
            <p class="mt-2 max-w-2xl text-sm leading-6 text-slate-500 dark:text-slate-400">
                <span x-show="lang === 'fa'">کارهای دارای موعد را روی تقویم ببین و مدیریت کن.</span>
                <span x-show="lang === 'en'">View and manage your tasks with due dates on the calendar.</span>
            </p>
        </div>
        ...
        <a href="{{ route('tasks') }}" class="inline-flex items-center justify-center gap-2 rounded-2xl bg-indigo-600 px-5 py-3 text-sm font-bold text-white shadow-lg shadow-indigo-500/20 transition hover:bg-indigo-700">
            ...
            <span x-show="lang === 'fa'">کار جدید</span>
            <span x-show="lang === 'en'">New Task</span>
        </a>
    </section>
    ...
    <section class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900">
        <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div class="flex flex-wrap items-center gap-2">
                <button type="button" wire:click="previousMonth" aria-label="Previous month" class="grid h-10 w-10 place-items-center rounded-xl bg-slate-100 text-slate-600 hover:bg-slate-200 dark:bg-slate-800 dark:text-slate-300">‹</button>
                <button type="button" wire:click="nextMonth" aria-label="Next month" class="grid h-10 w-10 place-items-center rounded-xl bg-slate-100 text-slate-600 hover:bg-slate-200 dark:bg-slate-800 dark:text-slate-300">›</button>
                <button type="button" wire:click="goToToday" class="rounded-xl bg-indigo-50 px-4 py-2.5 text-xs font-bold text-indigo-600 dark:bg-indigo-500/10 dark:text-indigo-400">
                    <span x-show="lang === 'fa'">امروز</span><span x-show="lang === 'en'">Today</span>
                </button>
    ...
    <div class="grid grid-cols-1 gap-6 xl:grid-cols-4">
        <section class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900 xl:col-span-3">
            <div class="grid grid-cols-7 border-b border-slate-200 dark:border-slate-800">
                @foreach ([['fa'=>'دوشنبه','en'=>'Monday'],['fa'=>'سه‌شنبه','en'=>'Tuesday'],['fa'=>'چهارشنبه','en'=>'Wednesday'],['fa'=>'پنجشنبه','en'=>'Thursday'],['fa'=>'جمعه','en'=>'Friday'],['fa'=>'شنبه','en'=>'Saturday'],['fa'=>'یکشنبه','en'=>'Sunday']] as $weekday)
                    <div class="py-3 text-center text-[11px] font-bold text-slate-400">
                        <span x-show="lang === 'fa'">{{ $weekday['fa'] }}</span>
                        <span x-show="lang === 'en'">{{ $weekday['en'] }}</span>
                    </div>
                @endforeach
            </div>
                    ...
                    @else
                        <button type="button" wire:click="selectDate('{{ $cell['date'] }}')" @class(['group min-h-28 border-b border-e border-slate-100 p-2 text-start transition hover:bg-slate-50 dark:border-slate-800 dark:hover:bg-slate-800/50','bg-indigo-50 ring-2 ring-inset ring-indigo-500 dark:bg-indigo-500/10' => $selectedDate === $cell['date']])>
                            ...
                            <div class="mt-2 space-y-1">
                                ...
                                @endforeach
                                @if ($cell['tasks']->count() > 3)<p class="px-1 text-[10px] font-bold text-slate-400">+{{ $cell['tasks']->count() - 3 }}</p>@endif
                            </div>
                        </button>
                    @endif
                @endforeach
        ...
        <aside class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
            <div class="mb-5">
                <p class="text-xs font-bold text-indigo-600 dark:text-indigo-400">{{ $selectedDate ? \Carbon\Carbon::parse($selectedDate)->translatedFormat('l, d F Y') : '—' }}</p>
                <h3 class="mt-1 text-xl font-black"><span x-show="lang === 'fa'">برنامه روز</span><span x-show="lang === 'en'">Day plan</span></h3>
            </div>
                            ...
                            <div class="min-w-0 flex-1">
                                <div class="flex items-center justify-between gap-2">
                                    ...
                                    @if ($task->priority === 'high')
                                        <span class="rounded-full bg-rose-50 px-2 py-1 text-[10px] font-bold text-rose-600 dark:bg-rose-500/10 dark:text-rose-400"><span x-show="lang === 'fa'">زیاد</span><span x-show="lang === 'en'">High</span></span>
                                    @elseif ($task->priority === 'medium')
                                        <span class="rounded-full bg-amber-50 px-2 py-1 text-[10px] font-bold text-amber-600 dark:bg-amber-500/10 dark:text-amber-400"><span x-show="lang === 'fa'">متوسط</span><span x-show="lang === 'en'">Medium</span></span>
                                    @else
                                        <span class="rounded-full bg-emerald-50 px-2 py-1 text-[10px] font-bold text-emerald-600 dark:bg-emerald-500/10 dark:text-emerald-400"><span x-show="lang === 'fa'">کم</span><span x-show="lang === 'en'">Low</span></span>
                                    @endif
                                </div>
                                <h4 @class(['mt-2 text-sm font-black','text-slate-400 line-through' => $task->status === 'completed'])>{{ $task->title }}</h4>
                                <p class="mt-1 text-xs text-slate-400">{{ $task->category?->name ?? '' }}@if (!$task->category)<span x-show="lang === 'fa'">بدون دسته</span><span x-show="lang === 'en'">No category</span>@endif</p>
                            </div>
                        </div>
                    </div>
                @empty
                    <div class="rounded-2xl border border-dashed border-slate-200 px-4 py-8 text-center dark:border-slate-700">
                        <p class="text-sm font-bold text-slate-600 dark:text-slate-300"><span x-show="lang === 'fa'">کاری برای این روز نیست</span><span x-show="lang === 'en'">No tasks for this day</span></p>
                        <p class="mt-1 text-xs text-slate-400"><span x-show="lang === 'fa'">یک کار با این تاریخ موعد بساز</span><span x-show="lang === 'en'">Create a task with this due date.</span></p>
                    </div>
                @endforelse

                @if ($selectedDate)
                    <form wire:submit="createTaskForSelectedDay" class="mt-5 space-y-3 border-t border-slate-100 pt-5 dark:border-slate-800">
                        <p class="text-xs font-bold text-slate-500"><span x-show="lang === 'fa'">کار جدید برای این روز</span><span x-show="lang === 'en'">New task for this day</span></p>
                        <input type="text" wire:model="quickTitle" placeholder="عنوان کار..." x-bind:placeholder="lang === 'fa' ? 'عنوان کار...' : 'Task title...'" class="h-11 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 text-sm outline-none focus:border-indigo-500 dark:border-slate-700 dark:bg-slate-800 dark:text-white">
                        @error('quickTitle')<p class="text-sm text-red-500">{{ $message }}</p>@enderror
                        <input type="text" wire:model="quickDescription" placeholder="توضیح کار..." x-bind:placeholder="lang === 'fa' ? 'توضیح کار...' : 'Task description...'" class="h-11 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 text-sm outline-none focus:border-indigo-500 dark:border-slate-700 dark:bg-slate-800 dark:text-white">
                        @error('quickDescription')<p class="text-sm text-red-500">{{ $message }}</p>@enderror
                        <select wire:model="quickPriority" class="h-11 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 text-sm dark:border-slate-700 dark:bg-slate-800 dark:text-white">
                        ...
                        <button type="submit" wire:loading.attr="disabled" wire:target="createTaskForSelectedDay" class="w-full rounded-xl bg-indigo-600 py-2.5 text-sm font-bold text-white transition hover:bg-indigo-700 disabled:opacity-60">
                            <span wire:loading.remove wire:target="createTaskForSelectedDay"><span x-show="lang === 'fa'">افزودن</span><span x-show="lang === 'en'">Add task</span></span>
