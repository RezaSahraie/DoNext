...
<div class="space-y-6" dir="{{ $isFa ? 'rtl' : 'ltr' }}">
    ...
    <section class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900">
        <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div class="flex flex-wrap items-center gap-2">
                <button type="button" wire:click="previousMonth" class="grid h-10 w-10 place-items-center rounded-xl bg-slate-100 text-slate-600 transition hover:bg-slate-200 dark:bg-slate-800 dark:text-slate-300 dark:hover:bg-slate-700">‹</button>
                <button type="button" wire:click="nextMonth" class="grid h-10 w-10 place-items-center rounded-xl bg-slate-100 text-slate-600 transition hover:bg-slate-200 dark:bg-slate-800 dark:text-slate-300 dark:hover:bg-slate-700">›</button>
    ...
    <div class="grid grid-cols-1 gap-6 xl:grid-cols-4">
        ...
        <aside class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
            <div class="mb-5">
                <p class="text-xs font-bold text-indigo-600 dark:text-indigo-400">{{ $selectedDate ? \Carbon\Carbon::parse($selectedDate)->locale($locale)->translatedFormat('l, d F Y') : '—' }}</p>
            ...
            <div class="space-y-3">
                    ...
                    <div class="rounded-2xl border border-slate-200 p-4 dark:border-slate-800">
                        <div class="flex items-start gap-3">
                            ...
                            <div class="min-w-0 flex-1">
                                ...
                                <p class="mt-1 text-xs text-slate-400">{{ $task->category?->name ?? ($isFa ? 'بدون دسته' : 'No category') }}</p>
                            </div>
                        </div>
                    </div>
                @empty
                ...
                @if ($selectedDate)
                    <form wire:submit="createTaskForSelectedDay" class="mt-5 space-y-3 border-t border-slate-100 pt-5 dark:border-slate-800">
                        ...
                        <input type="text" wire:model="quickTitle" placeholder="{{ $isFa ? 'عنوان کار...' : 'Task title...' }}" class="h-11 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 text-sm outline-none focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 dark:border-slate-700 dark:bg-slate-800 dark:text-white">
                        @error('quickTitle')<p class="text-sm text-red-500">{{ $message }}</p>@enderror
                        <input type="text" wire:model="quickDescription" placeholder="{{ $isFa ? 'توضیح کار...' : 'Task description...' }}" class="h-11 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 text-sm outline-none focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 dark:border-slate-700 dark:bg-slate-800 dark:text-white">
                        ...
                        <button type="submit" wire:loading.attr="disabled" wire:target="createTaskForSelectedDay" class="w-full rounded-xl bg-indigo-600 py-2.5 text-sm font-bold text-white transition hover:bg-indigo-700 disabled:opacity-60">
                            <span wire:loading.remove wire:target="createTaskForSelectedDay">{{ $isFa ? 'افزودن' : 'Add task' }}</span>
